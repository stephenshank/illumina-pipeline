...
def perform_initial_environment_check():
    ...
    required_cli_tools = [
        "bedtools", "bcftools", "bowtie2", "multiqc", "samtools",
        "trimmomatic", "varscan", "efetch", "seqkit", "sed",
        "csvcut", "perbase", "vapor.py", "mafft", "ivar"
    ]
    ...
    for module_name, display_name in required_python_modules.items():
        try:
            ...
            error_messages.append(f"  ❌ ERROR: Python module {display_name} ({module_name}) not found.")
            found_all_python_modules = False
        except ImportError as e:
            # Handle cases where the module is found, but fails during import
            error_messages.append(f"  ❌ ERROR: Failed importing Python module {display_name} ({module_name}).")
            error_messages.append(f"      └─> This often means an issue with its dependencies or environment setup.")
            error_messages.append(f"      └─> Original Error: {e}") # Include the original error message
            found_all_python_modules = False
        except Exception as e:
            # Catch any other totally unexpected error during import attempt
             error_messages.append(f"  ❌ ERROR: Unexpected error while trying to import {display_name} ({module_name}).")
    ...
    if not found_all_cli_tools or not found_all_python_modules:
        ...
        print("   Your environment does not appear to be correctly installed or activated.")
...
def command_line_interface():
    ...
    check_parser.set_defaults(func=check_cli) # No arguments specific to 'check' needed yet
...
def check_cli(args):
    ...
    if all_checks_clear:
        ...
        print("   Please review any ℹ️ informational or ⚠️ warning messages above for further context.")
...
def report_pipeline_status():
    ...
    try:
        ...
        essential_keys = ['reference', 'data_root_directory', 
                          'consensus_minimum_coverage', 'variants_minimum_coverage'] # Add more as needed
        missing_keys = [k for k in essential_keys if k not in config]
        if missing_keys:
            print_status_item(f"`config.yml` is missing essential keys: {', '.join(missing_keys)}", "error")
        ...
        if not data_root_val:
            print_status_item("`data_root_directory` not specified in `config.yml`.", "error")
            overall_status_ok = False
        else:
            data_root = Path(data_root_val).expanduser()
            if not data_root.exists():
                print_status_item(f"`data_root_directory` ('{data_root}') from `config.yml` does not exist.", "error")
                print_guidance("Please create this directory or correct the path in `config.yml`.")
                # This is a warning, might not block all subsequent checks but flow will fail.
            else:
                print_status_item(f"`data_root_directory` ('{data_root}') exists.", "success")

    except FileNotFoundError as e:
        print_status_item(f"`config.yml` not found.", "error")
        print_guidance(f"Please copy `config.yml.template` to `config.yml` and edit it.")
        overall_status_ok = False
    except (ValueError, yaml.YAMLError) as e: # Errors from load_mlip_config
        print_status_item(f"Error loading `config.yml`: {e}", "error")
        print_guidance("Please ensure `config.yml` is correctly formatted and not empty.")
        overall_status_ok = False
    except Exception as e:
    ...
    if config and 'reference' in config: 
        ...
        if reference_name_from_config == 'custom':
            ...
            custom_check_passed = True # Assume success for this specific check initially

            if not custom_ref_base_path.is_dir():
                print_status_item(f"Base directory for custom reference ('{custom_ref_base_path}') not found.", "error")
                print_guidance("Create this directory and place your custom reference segment subdirectories inside it.")
                print_guidance("This can be done manually (requires time and care), or by using unzipped one of our curated ZIP files.")
                ...
                if not segment_dirs:
                    print_status_item(f"No segment subdirectories found within '{custom_ref_base_path}'.", "error")
                    print_guidance("Create subdirectories for each segment (e.g., 'PB2', 'PB1', 'PA', etc.).")
                    print_guidance("Each needs 'sequence.fasta' and 'metadata.gb' files inside.")
                    custom_check_passed = False
                    overall_status_ok = False
                else:
                    segment_names = sorted([d.name for d in segment_dirs]) # Sort for consistent order
                    print_status_item(f"Found custom segment directories: {', '.join(segment_names)}", "info")
                    ...
                    for seg_dir in segment_dirs:
                        ...
                        if not fasta_path.exists():
                            missing_files_report.append(f"  - Segment '{segment_name}': Missing '{fasta_path.name}'")
                            custom_check_passed = False
                        if not genbank_path.exists():
                            missing_files_report.append(f"  - Segment '{segment_name}': Missing '{genbank_path.name}'") # Check for .gb
                            custom_check_passed = False

                    if not custom_check_passed:
                        print_status_item(f"Missing required files within custom reference directories:", "error")
                        # Print the collected missing file details
                        for report_line in missing_files_report:
                             print(report_line) 
                        print_guidance(f"Ensure each segment directory in '{custom_ref_base_path}' contains both 'sequence.fasta' and 'metadata.gb'.")
                        overall_status_ok = False # Critical failure if files missing
                    else:
                         print_status_item("Required files ('sequence.fasta', 'metadata.gb') found for all detected custom segments.", "success")
        
        # --- Handle non-custom references ---
        elif reference_name_from_config == 'h5n1': # Keep h5n1 bell check separate if desired
            print_status_item(f"You've selected '{reference_name_from_config}' as your reference.", "info")
            print_guidance("This uses a default H5N1 reference from GenBank.")
            print_guidance("For other GenBank references or custom setups, see documentation:")
            ...
            try:
                ...
                if not ref_details:
                    print_status_item(f"Definition for reference '{reference_name_from_config}' not found or invalid in 'references.tsv'.", "error")
                    overall_status_ok = False
                else:
                    print_status_item(f"Reference '{reference_name_from_config}' appears configured based on 'references.tsv'.", "success")
            except FileNotFoundError:
                print_status_item(f"'references.tsv' not found. Cannot verify reference '{reference_name_from_config}'.", "error")
                overall_status_ok = False
            except Exception as e:
                print_status_item(f"Unexpected error loading reference details for '{reference_name_from_config}': {e}", "error")
                overall_status_ok = False
            ...
            try:
                ref_details = load_reference_dictionary(reference_name_from_config)
                if not ref_details:
                    print_status_item(f"Definition for reference '{reference_name_from_config}' (from `config.yml`) not found or invalid in 'references.tsv'.", "error")
                    print_guidance(f"Ensure '{reference_name_from_config}' is a valid entry in the 'virus' column of `references.tsv`.")
                    overall_status_ok = False
                else:
                    print_status_item(f"Reference '{reference_name_from_config}' appears configured based on 'references.tsv'.", "success")
            except FileNotFoundError:
                print_status_item(f"'references.tsv' not found. Cannot verify reference '{reference_name_from_config}'.", "error")
                print_guidance("Please ensure 'references.tsv' exists in the current directory.")
                overall_status_ok = False
            except Exception as e:
                print_status_item(f"Unexpected error loading reference details for '{reference_name_from_config}': {e}", "error")
                overall_status_ok = False
    ...
    # --- Handle cases where config is missing or 'reference' key is missing ---
        ...
        print_status_item("Skipping reference check because `config.yml` is not loaded.", "info")
        ...
        print_status_item("`reference` key missing in `config.yml`, cannot check reference setup.", "error")
        overall_status_ok = False
    ...
    if not metadata_path.exists():
        ...
        print_guidance("This file is generated by: `python mlip/dataflow.py preprocess -f <your_id_list.txt>`.")
        print("")
        print_guidance("Alternatively, you may want to use an existing metadata spreadsheet.")
        ...
        try:
            df = pd.read_csv(metadata_path, sep='\t', dtype=str).fillna('') # Read all as str, fill NaN with empty str
            if df.empty and not list(df.columns): # No columns means truly empty file or just whitespace
                print_status_item("`data/metadata.tsv` is an empty file (no headers, no data).", "warning")
                print_guidance("Run `preprocess` if you haven't, or ensure it's correctly formatted.")
            elif df.iloc[:, 0].eq('').all() and len(df.columns) <=1 : # Heuristic for empty content with just headers
                 print_status_item("`data/metadata.tsv` appears to have headers but no data rows.", "warning")
                ...
                if missing_cols:
                    print_status_item(f"`data/metadata.tsv` is missing required columns: {', '.join(missing_cols)}.", "error")
                    overall_status_ok = False
                else:
                    print_status_item("Required columns found in `data/metadata.tsv`.", "success")
                    # Check for non-empty SampleId and Replicate columns in data rows
                    if df.empty: # Only headers, no data rows
                        print_status_item("`data/metadata.tsv` has headers but no data rows.", "warning")
                        print_guidance("Please populate it with your sample information.")
                        overall_status_ok = False
                    elif df['SampleId'].eq('').any() or df['Replicate'].eq('').any():
                        print_status_item("`data/metadata.tsv` has some empty values in 'SampleId' or 'Replicate'.", "error")
                        print_guidance("Ensure these are fully populated for all intended samples.")
                        overall_status_ok = False
                    else:
                        print_status_item("`data/metadata.tsv` appears populated.", "success")
        except pd.errors.EmptyDataError:
            print_status_item("`data/metadata.tsv` exists but is completely empty (cannot be parsed).", "error")
            print_guidance("Run `python mlip/dataflow.py preprocess -h` and follow instructions therein.")
            overall_status_ok = False
        except Exception as e:
            print_status_item(f"Error reading or parsing `data/metadata.tsv`: {e}", "error")
            overall_status_ok = False
